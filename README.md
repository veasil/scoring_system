...
## 🛠️ 配置先行开发规范 (Configuration-Driven Development)

本系统采用**配置先行**的设计理念。为了提高系统的灵活性和可维护性，所有业务逻辑参数、安全密钥以及品牌信息都应通过 `system_settings` 表进行管理，而不是硬编码在代码中。

### 规范要点：
1. **参数持久化**：新的业务参数（如游戏倒计时、属性上限）必须存入 `system_settings` 表。
2. **后端分发**：前端通过 `/api/settings` 接口动态获取配置，实现“修改后台配置，全系统即时生效”。
3. **安全隔离**：敏感密钥（如 `DEV_KEY`）由后端校验，严禁在前端硬编码。
4. **业务动态化**：游戏模式（卡牌分布）应通过 JSON 配置定义，支持在不修改代码的情况下新增或调整模式。
