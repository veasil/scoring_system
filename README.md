...
> 《AI在5000天·伍力全开》儿童安全教育桌游的数字化计分平台

玩家（守望者）通过微信扫码或手机号登录，参与情境卡牌游戏。每次选择影响五维属性值：安全力 / 脑波力 / 实感力 / 创心力 / 沟通力。运营人员通过管理后台管理卡牌内容、用户等级和活动场次。
...
登录：`admin` 账号使用 `DEV_KEY`；普通用户需 `role=boss` 才可进入。
